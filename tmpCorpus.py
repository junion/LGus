...
class Dialog:
    ...
    '''
    def __old_init__(self,filename,max_turn_num=40):
        self.id = filename
...
#            elif line.find('Success') > -1 and line.find('Arrival') > -1:
#                potential_field = 'ap'
            if parsing_state.find('expect_goal') < 0:
                # system part
                if line.find('object\thow_to_get_help') > -1:
                    ...
                    turn = {'SA':['R:tt']}
                elif line.find('object\texact_travel_time') > -1:
                    turn = {'SA':['R:tt']}
    ...
    #            elif line.find('[[generic.yes]]') > -1:
                elif line.find(':[generic.yes]') > -1 or line.find('  [generic.yes]') > -1:
                    turn['UA'].append('yes')
                    try:
    ...
    #            elif line.find('[[generic.no]]') > -1:
                elif line.find(':[generic.no]') > -1 or line.find('  [generic.no]') > -1:
                    turn['UA'].append('no')
    ...
    #                print line
                    ...
                    parsing_state = ''
            # backend query
            if line.find('Make query for schedule') > -1 or\
...
#            elif line.find('[[generic.quit]]') > -1:
            elif line.find(':[generic.quit]') > -1 or line.find('  [generic.quit]') > -1:
                print 'quit'
                break
