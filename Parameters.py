...
class Factor(SubDomain):
    ...
    def map_marginalise_away(self,variables):
        variables = frozenset(variables)
        self._data = self._map_data_marginalise(
            self._data,
            sorted(self._variables),
            self._variables & variables)
        SubDomain.marginalise_away(self,variables)
    ...
    def _map_data_marginalise(self,data,variables,to_maxout):
        """Sum out variables C{to_maxout} from C{data}

        Variables in C{to_maxout} not in C{variables} have no
        effect
        @param data: Data
        @type data: List
        @param variables: The (ordered) variables for C{data}
        @type variables: Sequence
        @param to_maxout: Variables to sum out
        @type to_maxout: Set/frozenset
        @return: Data with C{to_maxout} variables summed out
        @rtype: List
        """
        if not to_maxout:
            return data
        if to_maxout.issuperset(variables):
            return [reduce(max,data)]
        variable, rest = variables[0], variables[1:]
        chunk_size = len(data)/self._numvals[variable]
        if variable in to_maxout:
            new_to_maxout = to_maxout - set([variable])
            newdata = self._data_marginalise(data[:chunk_size],
                                                   rest,
                                                   new_to_maxout)
            for i in range(chunk_size,len(data),chunk_size):
            ...
            for i in range(0,len(data),chunk_size):
                newdata.extend(
                    self._map_data_marginalise(data[i:i+chunk_size],rest,to_maxout))
        return newdata
...
#            return data,[]
#        if to_maxout.issuperset(variables):
#            max_item = max(zip(data,self.varinsts(variables)).iteritems(),key=operator.itemgetter(0))
...
#        if variable in to_maxout:
#            new_to_maxout = to_maxout - set([variable])
#            values = sorted(self._domain[variable]);v = 0
#            newdata,newtb = self._map_data_marginalise(data[:chunk_size],
#                                                   rest,
#                                                   new_to_maxout)
#            for k in range(len(newtb)):
#                newtb[k][variable] = values[v]
#            for i in range(chunk_size,len(data),chunk_size):
#                d,t = self._map_data_marginalise(data[i:i+chunk_size],rest,new_to_maxout);v += 1
#                for k in range(len(newtb)):
#                    if d[k] > newdata[k]:
#                        newdata[k] = d[k]
#                        newtb[k][variable] = values[v]
...
#            newdata = [], newtb = []
#            for i in range(0,len(data),chunk_size):
#                d,t = self._map_data_marginalise(data[i:i+chunk_size],rest,to_maxout)
#                newdata.extend(d); newtb.extend(t)
#        return newdata,newtb
